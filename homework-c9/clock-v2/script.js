...
for (let i = 1; i <= 60; i++) {
    if (i % 5 == 0) {
        const displayHours = [12, 3, 6, 9];
        if (displayHours.includes(i / 5)) { // a se vedea includes din Array https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Array/includes
            time.innerHTML +=
            ...
            time.innerHTML +=
                "<div class='hour-number'><div></div></div>";
            let hours = document.getElementsByClassName("hour-number");
            hours[hours.length - 1].style.transform = `translateX(-50%) rotate(${
                i * 6
            }deg)`;
